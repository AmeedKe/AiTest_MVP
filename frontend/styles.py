...
def setup_page():
    st.set_page_config(page_title="AiTest", page_icon="🪄", layout="centered")
    css = _CSS_PATH.read_text(encoding="utf-8")
    st.markdown(f"<style>{css}</style>", unsafe_allow_html=True)
    st.markdown("<h1>AiTest ✨</h1>", unsafe_allow_html=True)
...
def question_box(topic, question):
    st.markdown(
        ...
        border-right:5px solid #6c5ce7;font-size:1.3rem;white-space: pre-line;'>
